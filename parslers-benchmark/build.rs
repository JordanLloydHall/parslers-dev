...
#[reflect]
fn zip_prog(a: Vec<parslers_wacc::ASTWrapper<String, parslers_wacc::Function<String, String>>>) -> Box<dyn FnOnce(Vec<parslers_wacc::ASTWrapper<String, parslers_wacc::Stat<String, String>>>) -> (Vec<parslers_wacc::ASTWrapper<String, parslers_wacc::Function<String, String>>>, Vec<parslers_wacc::ASTWrapper<String, parslers_wacc::Stat<String, String>>>)> {
    Box::new(move |b| (a, b))
}

#[reflect]
fn bridge_program((a, b): (Vec<parslers_wacc::ASTWrapper<String, parslers_wacc::Function<String, String>>>, Vec<parslers_wacc::ASTWrapper<String, parslers_wacc::Stat<String, String>>>)) -> parslers_wacc::Program<String, String> {
    parslers_wacc::Program(a, b)
}

fn wws<O>(p: impl Parsler<Output = O> + Clone) -> impl Parsler<Output = O> + Clone {
    p.before(many(one_of(" \t\n\r".chars()).then(pure(())).or(match_char('#').then(many(not_one_of("\n\r".chars()))).then(pure(())))))
}
...
fn wacc_program() -> impl Parsler<Output = parslers_wacc::Program<String, String>> + Clone {
    wws(pure(())).then(wtag("begin")).then(pure(vec![])).map(zip_prog).ap(wacc_stats(wtag("end"))).map(bridge_program)
    // Empty::default()
}

fn wacc_stats(p: impl Parsler + Clone) -> impl Parsler<Output = Vec<parslers_wacc::ASTWrapper<String, parslers_wacc::Stat<String, String>>>> + Clone {
    (many(wacc_stat().before(opt(ws(match_char(';'))))).map(append)
    .ap(wacc_stat().map(option).or(pure(None)))).before(p)
}
...
#[reflect]
fn wacc_exit(a: parslers_wacc::ExprWrap<String, String>) -> parslers_wacc::ASTWrapper<String, parslers_wacc::Stat<String, String>> {
    parslers_wacc::ASTWrapper(String::new(), parslers_wacc::Stat::Exit(a))
}

fn wacc_stat() -> impl Parsler<Output = parslers_wacc::ASTWrapper<String, parslers_wacc::Stat<String, String>>> + Clone {
    wtag("skip").then(pure(wrapp(parslers_wacc::Stat::Skip))).or(wtag("exit").then(wacc_expr()).map(wacc_exit))
}
